...
export function eventCardTextScale(card: EventCard, includeDescription: boolean): EventCardTextScale {
  ...
  if (includeDescription) {
    if (length > 380) return { bodyFontSize: '0.74rem', effectIconSize: 30, rowSpacing: 0.6 };
    if (length > 300) return { bodyFontSize: '0.8rem', effectIconSize: 32, rowSpacing: 0.8 };
    if (length > 220) return { bodyFontSize: '0.84rem', effectIconSize: 33, rowSpacing: 0.9 };
